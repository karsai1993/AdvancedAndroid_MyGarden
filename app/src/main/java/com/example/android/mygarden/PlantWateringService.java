...
public class PlantWateringService extends IntentService {
    ...
    private void handleActionUpdatePlantWidgets() {
        ...
        if (cursor != null && cursor.getCount() > 0) {
            ...
            plantId = cursor.getLong(
                    cursor.getColumnIndex(PlantContract.PlantEntry._ID)
            );
            cursor.close();
            waterAge = timeNow - wateredAt;
            canWater = waterAge > PlantUtils.MIN_AGE_BETWEEN_WATER && waterAge < PlantUtils.MAX_AGE_WITHOUT_WATER;
            imgRes = PlantUtils.getPlantImageRes(this, timeNow - createdAt, waterAge, plantType);
        }
